...
public class SysUserController extends BaseController
{
    ...
    {
        ExcelUtil<SysUser> util = new ExcelUtil<SysUser>(SysUser.class);
        return util.importTemplateExcel("学生认证数据");
    }
    ...
    public AjaxResult certification(@Validated @RequestBody SysUser user)
    {
        SysUser query=new SysUser();
        query.setNickName(user.getNickName());
        query.setIdNumber(user.getIdNumber());
        List<SysUser> userlist=userService.selectUserList(query);
        if(!CollectionUtils.isEmpty(userlist)){
            for (SysUser item:userlist) {
                item.setStatus("1");
                userService.updateUserStatus(item);
            }
